...
public class ComponentLine extends AbstractLine<Component> {

  private final boolean clickable;
  protected AABB hitbox;
  private boolean isEmpty;

  public ComponentLine(@NotNull Hologram hologram, @NotNull Component obj, boolean clickable) {
    super(hologram, obj);
   ...
   */
  @NotNull
  public Component parse(Player player) {
  ...
  protected void setLocation(@NotNull Location location) {
    ...
    if (clickable) {
      double chars = LegacyComponentSerializer.legacySection().serialize(obj).length();
      double size = 0.105;
  ...
  private static boolean isEmpty(@NotNull Component component) {
    return component instanceof TextComponent && ((TextComponent) component).content().isEmpty();
  }
}
